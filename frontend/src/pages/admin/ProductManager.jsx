  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-warm-900 font-heading">Products</h1>
          <p className="text-xs sm:text-sm text-warm-600">Manage your store's inventory and listings.</p>
        </div>
        <Link 
          to="/admin/products/new" 
          className="w-full sm:w-auto justify-center inline-flex items-center px-4 py-2.5 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-brand-700 hover:bg-brand-800 focus:outline-none"
        >
      ...
      <div className="bg-white rounded-lg sm:rounded-xl shadow-sm border border-warm-200 overflow-hidden">
        {isLoading ? (
          <div className="p-8 sm:p-12 flex justify-center">
            <LoadingSpinner size="lg" />
          </div>
        ) : products.length === 0 ? (
          <div className="p-8 sm:p-12 text-center text-warm-500 text-sm sm:text-base">
            No products found. Start by adding a new one!
          </div>
        ) : (
          <>
            {/* ── Mobile Card View (< md) ── */}
            <div className="block md:hidden divide-y divide-warm-200">
              {products.map((product) => {
                const primaryImg = product.images?.find(i => i.isPrimary) || product.images?.[0];
                const imgUrl = primaryImg?.processedUrl || primaryImg?.originalUrl || 'https://via.placeholder.com/60';

                return (
                  <div key={product.id} className="p-4 flex flex-col gap-3">
                    <div className="flex items-start gap-3">
                      <div className="flex-shrink-0 h-14 w-14 bg-warm-100 rounded-md overflow-hidden border border-warm-200">
                        <img className="h-full w-full object-cover" src={imgUrl} alt={product.name} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="text-sm font-medium text-warm-900 truncate">{product.name}</h4>
                        <p className="text-xs text-warm-500 mt-0.5">{product.category?.name || 'Uncategorized'}</p>
                        <div className="flex items-center gap-2 mt-1.5">
                          <span className="text-sm font-semibold text-brand-800">
                            {product.price} {product.currency || 'EGP'}
                          </span>
                          <span className="text-xs text-warm-400">·</span>
                          <span className={`text-xs ${product.stock > 0 ? 'text-warm-600' : 'text-red-600 font-medium'}`}>
                            Stock: {product.stock}
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-warm-100">
                      ...
                      <div className="flex items-center gap-1">
                        <a 
                          ...
                          target="_blank" 
                          rel="noreferrer"
                          className="p-2 text-warm-500 hover:text-brand-600 rounded-md"
                          title="View on Store"
                        ...
                        <Link 
                          ...
                          className="p-2 text-brand-600 hover:text-brand-900 rounded-md"
                          title="Edit"
